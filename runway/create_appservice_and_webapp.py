...
class CreateAppserviceAndWebapp(DeploymentStep):
    def __init__(self, env: ApplicationVersion, config: dict):
        ...
        self.runway_azure = self.config["runway_azure"]

    def run(self):
        self.create_appservice_and_webapp()

    def _create_or_update_appservice(
        self, web_client: WebSiteManagementClient, dtap: str, service_to_create: AppService
    ) -> str:
        service_plan_async_operation = web_client.app_service_plans.create_or_update(
            self.runway_azure["resource_group"].format(dtap=dtap.lower()),
            service_to_create.name,
            AppServicePlan(
                app_service_plan_name=service_to_create.name,
                location=self.runway_azure["location"],
    ...
    def _parse_appservice_parameters(self, dtap: str) -> AppService:
        """
        Parse parameters to use from environment variables. Defaults are provided for all required SKU
        parameters and for the location
        ...
        return AppService(
            ...
            sku=AppServiceSKU(name=sku_config.name, capacity=sku_config.capacity, tier=sku_config.tier),
        )
    ...
    def _build_site_config(self, existing_properties: dict) -> SiteConfig:
        ...
        new_properties = {
            ...
            "DOCKER_REGISTRY_SERVER_URL": "https://" + self.config["runway_common"]["shared_registry"],
        ...
        # This should make sure that properties set by other parties and other deployment scripts will not be
        # removed as this is the default Azure behaviour
    ...
    def _get_webapp_to_create(self, appservice_id: str, web_client: WebSiteManagementClient) -> WebApp:
        # use build definition name as default web app name
        application_name = get_application_name()
        formatted_dtap = self.env.environment.lower()

        webapp_name = "{name}-{env}".format(name=application_name.lower(), env=formatted_dtap)

        existing_properties = {}
        try:
            existing_properties = web_client.web_apps.list_application_settings(
                self.runway_azure["resource_group"].format(dtap=formatted_dtap), webapp_name
            ).properties
        except CloudError:
            logging.warning(f"{webapp_name} could not be found, skipping existing properties")

        return WebApp(
            resource_group=self.runway_azure["resource_group"].format(dtap=formatted_dtap),
            name=webapp_name,
            site=Site(
                location=self.runway_azure["location"],
    ...
    def _get_appservice(self, web_client: WebSiteManagementClient, dtap: str) -> str:
    ...
    def _create_or_update_webapp(self, web_client: WebSiteManagementClient, webapp_to_create: WebApp) -> Site:

        return web_client.web_apps.create_or_update(
            webapp_to_create.resource_group, webapp_to_create.name, webapp_to_create.site
        )
    ...
    def create_appservice_and_webapp(self) -> Site:
        ...
        appservice_id = self._get_appservice(web_client=web_client, dtap=formatted_dtap)

        webapp_to_create = self._get_webapp_to_create(appservice_id=appservice_id, web_client=web_client)

        site = self._create_or_update_webapp(web_client=web_client, webapp_to_create=webapp_to_create)
        ...
        web_client.web_apps.restart(
            resource_group_name=(self.runway_azure["resource_group"].format(dtap=formatted_dtap)),
