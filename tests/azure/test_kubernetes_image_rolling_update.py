...
def victim(_):
    conf = {
        **takeoff_config(),
        **{
            "task": "kubernetes_image_rolling_update",
    ...
    with mock.patch(
        ...
        return KubernetesImageRollingUpdate(
            ApplicationVersion("dev", "1.0.2", "branch"), conf
        )
...
class TestKubernetesImageRollingUpdate(object):
    ...
    @mock.patch(
        "takeoff.azure.kubernetes_image_rolling_update.run_shell_command",
        return_value=0,
    )
    def test_apply_rolling_update_success(
        self, m_bash, victim: KubernetesImageRollingUpdate
    ):
    ...
    def test_apply_rolling_update_failure(
        self, m_bash, victim: KubernetesImageRollingUpdate
    ):
