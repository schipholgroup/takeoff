...
class TestPublishArtifact(unittest.TestCase):
    ...
    def test_get_jar_too_many(self):
        with mock.patch.object(
            glob,
            "glob",
            return_value=["some-assembly-file.jar", "another-assembly-shizzle.jar"],
        ):
    ...
    def test_get_wheel_too_many(self):
        with mock.patch.object(glob, "glob", return_value=["some.whl", "another.whl"]):
            with pytest.raises(FileNotFoundError):
                victim._get_jar()
    ...
    def test_upload_file_to_blob(self, _, __):
        ...
        m.create_blob_from_path.assert_called_once_with(
            container_name="mylittlepony", blob_name="Mustaine", file_path="Dave"
        )
    ...
    def test_publish_to_ivy(self, _, __, ___):
        conf = {**takeoff_config(), **BASE_CONF, "language": "scala", "target": ["ivy"]}
        with mock.patch(
            "takeoff.azure.publish_artifact.run_shell_command", return_value=0
        ) as m:
